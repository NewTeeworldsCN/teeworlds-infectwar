...
int net_addr_from_str(NETADDR *addr, const char *string)
{
	...
	if(str[0] == '[')
	{
...
#if defined(CONF_FAMILY_WINDOWS)
		{
			...
			size = (int)sizeof(sa6);
			if(WSAStringToAddress(buf, AF_INET6, nullptr, (struct sockaddr *)&sa6, &size) != 0)
				return -1;
		}
#else
...
void gui_messagebox(const char *title, const char *message)
{
	...
	CreateStandardAlert(kAlertStopAlert,
			CFStringCreateWithCString(nullptr, title, kCFStringEncodingASCII),
			CFStringCreateWithCString(nullptr, message, kCFStringEncodingASCII),
			nullptr,
			&theItem);

	RunStandardAlert(theItem, nullptr, &itemIndex);
